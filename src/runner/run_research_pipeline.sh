...
if [ -f "pyproject.toml" ]; then
    EXEC_CMD="(command -v uv || pip install uv --user >/dev/null 2>&1) && if [ \"\$(uname -m)\" = \"aarch64\" ]; then echo \"⚠️ ARM64 architecture detected. Using system packages to preserve L4T PyTorch...\"; rm -rf .venv && uv venv --python /usr/bin/python3 --system-site-packages --seed && uv run python3 -m pip install . && uv run dvc repro $DVC_ARGS; else uv sync && uv run dvc repro $DVC_ARGS; fi"
else
