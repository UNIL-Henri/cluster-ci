...
7. **Configuration `.cluster-ci`**: Les jobs nécessitant d'être schedulés peuvent déclarer une contrainte de RAM (ex: `REQUIRED_RAM=16GB` dans le fichier `.cluster-ci` à la racine). Ce paramètre est une contrainte de placement (pour atterrir sur un nœud suffisamment puissant) et non une limite stricte. Une fois alloué, le conteneur a accès à 100% de la RAM hôte pour éviter les limites artificielles.
## Main Results
